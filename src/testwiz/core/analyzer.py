...
class Analyzer:
    ...
    def find_class_by_name(self, source_file_path: str, class_name: str) -> List[Any]:
        """
        Finds a class by its name and returns the start and end lines of the class.
        ...
        for node, tag in captures:
            if tag == "definition.class":
                if self._is_class_name(node, class_name, source_code):
                    return node
        raise ValueError(f"Class {class_name} not found in file {source_file_path}")
