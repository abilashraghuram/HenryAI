...
from testwiz.core.analyzer import Analyzer
from testwiz.core.coverage_processor import CoverageProcessor
from testwiz.core.entities.config import UnittestGeneratorConfig
from testwiz.core.error_parser import extract_error_message
from testwiz.core.prompts.user import (
    FAILED_TESTS_TEXT,
...
from testwiz.core.router import LLMRouter
from testwiz.core.runner import TestRunner
...
class UnittestGenerator:
    def __init__(self, config: UnittestGeneratorConfig):
        ...
        self.octo_client = OctoAI()

        self.target_line_coverage_rate = 0.9
        self.target_mutation_coverage_rate = 0.9
        self.max_attempt = 4
    ...
    def run(self) -> None:
        ...
        while (
            self.coverage_processor.line_coverage_rate < self.target_line_coverage_rate
            and current_attempt_for_line < self.max_attempt
        ):
        ...
        while (
            self.current_mutationn_coverage < self.target_mutation_coverage_rate
            and current_attempt_for_mutation < self.max_attempt
        ):
        ...
        completion = self.octo_client.text_gen.create_chat_completion(
            model="wizardlm-2-8x22b",
            messages=[
                ChatMessage(role="user", content=user_template),
            ],
            max_tokens=4096,
        )
